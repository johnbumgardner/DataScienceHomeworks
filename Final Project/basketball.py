...
for teamInput in NBA:
    try: #Need this because teams like the Hornets that change name & location every 3 years break everything
        #%% Choose a team 
        ...
        for season, ignore in teamAdvancedSummary.items():
        ...
        print("{} broke it because they change location/name too much, excluded in allMSECorr")

#%% The program runs for almost an hour before getting team data to fill this, DO NOT OVERWRITE!
#allMSECorr.to_excel("allMSECorr.xlsx")

allMSECorr = pd.read_excel("allMSECorr.xlsx", index_col=0)

sumAllMSECorr = allMSECorr.sum(axis=0) #Sum each column over all teams
sumAllMSECorr = sumAllMSECorr[1:] #Remove team row, just want data
avgAllMSECorr = sumAllMSECorr / len(allMSECorr)
